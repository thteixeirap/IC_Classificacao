...
results_dir = 'Regression/Loan Approval/results_linear_regression'
...
def analisar_dados(X, y, df):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'correlacao_features.png'))
    plt.close()
    ...
    plt.ylabel('Contagem')
    plt.savefig(os.path.join(results_dir, 'distribuicao_target.png'))
    plt.close()
    ...
    for coluna in X.columns:
        ...
        plt.xlabel(coluna)
        plt.ylabel('Contagem')
        plt.savefig(os.path.join(results_dir, f'distribuicao_{coluna}.png'))
        plt.close()
...
def analisar_residuos(y_test, y_pred):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'analise_residuos.png'))
    plt.close()
...
def treinar_modelo(X, y, n_repeticoes=10):
    ...
    plt.savefig(os.path.join(results_dir, 'boxplot_metricas.png'))
    plt.close()
...
def plotar_predicoes(y_test, y_pred):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'predicoes.png'))
    plt.close()
