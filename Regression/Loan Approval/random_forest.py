...
results_dir = 'Regression/Loan Approval/results_random_forest'
...
def analisar_dados(X, y, df):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'rf_correlacao_features.png'))
    plt.close()
    ...
    plt.ylabel('Contagem')
    plt.savefig(os.path.join(results_dir, 'rf_distribuicao_target.png'))
    plt.close()
    ...
    for coluna in X.columns:
        ...
        plt.xlabel(coluna)
        plt.ylabel('Contagem')
        plt.savefig(os.path.join(results_dir, f'rf_distribuicao_{coluna}.png'))
        plt.close()
...
def treinar_modelo(X, y, n_repeticoes=10):
    ...
    plt.savefig(os.path.join(results_dir, 'rf_boxplot_metricas.png'))
    plt.close()
