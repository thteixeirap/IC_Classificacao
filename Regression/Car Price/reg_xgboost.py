...
results_dir = 'Regression/Car Price/results_xgboost'
...
def carregar_dados():
    ...
    for col in categoricas:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(results_dir, f'xgb_distribuicao_{col.lower()}.png'))
        plt.close()
    ...
    plt.savefig(os.path.join(results_dir, 'xgb_preco_boxplot.png'))
    plt.close()
    ...
    plt.savefig(os.path.join(results_dir, 'xgb_ano_preco_scatter.png'))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'xgb_matriz_correlacao.png'))
    plt.close()
...
def plotar_importancia_features(modelo, X, n_top=20):
    ...
    plt.title(f'Top {n_top} Features Mais Importantes - XGBoost')
...
def treinar_modelo(X, y, n_repeticoes=30):
    """
    Treina e avalia o modelo XGBoost com múltiplas repetições
    """
